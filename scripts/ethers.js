...
const oldMESAddress = "0x984b6968132DA160122ddfddcc4461C995741513";
const oldMESAbi = () => {
...
const oldMES = new ethers.Contract(oldMESAddress, oldMESAbi(), signer);
...
const getMESBalance = async(address_) => {
    if (migrated) {
        ...
        return formatEther( (await oldMES.balanceOf(address_)) );
    }
};
...
const getMESCredits = async(address_) => {
    if (migrated) {
        ...
        return formatEther( (await oldMES.getTotalClaimableTokens(address_)) );
    }
};
...
const getMESYieldRate = async(address_) => {
    if (migrated) {
        ...
        return formatEther( (await oldMES.getYieldRateOfAddress(address_)) );
    }
};
...
const claimMEScredits = async() => {
    if (migrated) {
        ...
        await displayErrorMessage("Error: You must migrate your $MES!")
    }

}


const migrateMEScredits = async() => {
    if (migrated) {
...
const updateInfo = async() => {
    ...
    if (migrated) {
        ...
        button = `<span id="migrate-button" onclick="migrateMEScredits()">MIGRATE</span>`;
    }
    $("#your-mes-credits").html(`${await getMESCredits(_address)} <img src="./images/mes.png" width="30px"><div style="height:5px" class="hide-on-desktop"></div>${button}`);
